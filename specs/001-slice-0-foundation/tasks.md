...
- [ ] T046 [US1] Frontend tests, in `apps/web/src/features/health/HealthStatus.test.tsx` (depends on T045; FR-023, SC-010, PLT-011 — **corrected this pass so the task concretely requires, not merely labels, every behavior Principle VII cites**), covering explicitly:
  ...
  - The existing `jest-axe` accessibility assertion (`toHaveNoViolations`, T008's setup; plain Testing-Library/DOM queries elsewhere, not jest-dom matchers).
...
- [ ] T060 [US5] Register the `/api/ws` upgrade route into the Axum router in `crates/service/src/main.rs` — in `crates/service/src/main.rs` (depends on T057, T058, T059, and T037 — same file as T031/T035/T036/T037, sequential, see File-Ownership Map; FR-022, SC-009)
...
**`SC-007` has two distinct forms of evidence in this task list (clarified this pass)** — spec.md's SC-007 covers visibility of failures in authentication denial, architectural boundaries, migrations, and tests. Conflating "a runbook exists" with "the gate actually made a failure visible" was corrected in the third revision; this pass makes the two forms explicit rather than crediting `SC-007` to more rows:

1. **Automated category coverage** — cited here narratively, not by adding the `SC-007` identifier to further rows (doing so would re-introduce the exact inflation this pass corrects for `T025`/`T026`/`T036`/`T037`/`T059`). `T021`/`T032` (HTTP auth-denial detection) and `T053` (WS auth-denial detection, `PENDING`) demonstrate the authentication category; `T049`/`T050` demonstrate the architectural-boundary category, with `T050`'s synthetic-violation fixture concretely proving the check reports a violation rather than passing silently; `T067`/`T070`/`T078` demonstrate that a fold/rebuild failure surfaces as a typed error and rolls back rather than silently succeeding, covering the "test"/data-integrity category. **This task list has no automated test that deliberately breaks a migration file and asserts the build/CI visibly fails** — that specific sub-category is not automated in this slice, consistent with the instruction that `T084` need not require one manual failure per category; it is exercised only by `T084`'s single human-performed check.
2. **Human checkpoint (`T084`)** — the sole task the `SC-007` identifier is credited to below. It is the only evidence that the complete local (`just check`) and CI (`T081`'s workflow) gates make an intentionally introduced failure visible end-to-end, and it remains `PENDING` until the human maintainer supplies that evidence.
...
| T029 | [impl] | FR-019, FR-020 | — | AC-043 | — | IV *(runs `contracts-generate`; the zero-drift check `SC-008` requires stays exclusively on T085, corrected this pass)* |
...
| T040, T042 | [impl] | FR-023 | SC-010 | — | PLT-011 | VII |
| T041 | [impl] | FR-024 | SC-011 | — | PLT-004 | VII |
| T043 | [impl] | FR-024 | SC-011 | — | PLT-004 | II *(mounts `QueryClient`; does not itself reach backend behavior — corrected this pass, `FR-008` removed)* |
| T044 | [impl] | FR-008, FR-024 | SC-011 | AC-035 | PLT-004 | II *(the task that concretely reaches the backend through a typed client)* |
| T045 | [impl] | FR-023 | SC-010 | — | PLT-011 | VII |
...
| T049, T050 | [test]/[infra] | FR-009, FR-010 | SC-005 | AC-041 | — | II, VI *(the check itself and its own test — this is where FR-010's "repeatable, automatable procedure" concretely lives)* |
...
| T056 | [impl] | FR-019, FR-020 | — | AC-043 | — | IV *(runs `contracts-generate`; `SC-008`'s zero-drift evidence stays exclusively on T085, corrected this pass)* |
| T057, T058 | [impl] | FR-022 | SC-009 | AC-033, AC-034 | SEC-001, SEC-003 | III, VI *(RGR pairing with T053/T054 is PENDING)* |
...
| T060 | [impl] | FR-022 | SC-009 | AC-033, AC-034 | SEC-001, SEC-003 | II, III |
...
| T068, T071 | [test]/[impl] | FR-011 | — | AC-042 | PLT-002 | V |
| T069, T077 | [test]/[impl] | FR-012 | SC-003 | AC-042 | PLT-002 | V |
...
| VII. Product and UX Integrity | **PASS** | T040–T042 make Graphite Signal tokens, theme selection, and reduced-motion CSS handling explicit tasks with exact file paths; T045 implements the Frontend State Matrix consuming only those tokens/rules. T046 (**corrected this pass to concretely require, not merely label, this evidence**) tests: every applicable Frontend State Matrix state rendered under both Light and Dark themes; a simulated `prefers-reduced-motion: reduce` query asserting T042's motion rules suppress non-essential movement while state is still communicated immediately; keyboard navigation and visible focus; non-color state communication; and the existing `jest-axe` assertion — `expect` correctly imported (T008), no unpinned `jest-dom` dependency, no new package added. |
...
| IX. Documentation and Delivery Discipline | **PENDING** | T010/T080 make the `justfile` the single command facade, each now credited only with the specific recipe surface it concretely completes (corrected this pass); T081 is the actual CI-level evidence for `SC-006`/`AC-044`/`PLT-005`/`PLT-009`; T083 documents the CI-break runbook. **`SC-007`'s automated-category coverage** (authentication: T021/T032/T053; architectural boundary: T049/T050; data/test-integrity: T067/T070/T078 — see the note above the traceability matrix) exists and is otherwise executable, but does not by itself satisfy `SC-007`. **Blocker**: `SC-007`'s human-checkpoint form requires T084's execution evidence, which does not yet exist — this principle remains `PENDING` until that evidence is recorded. |
| X. Human-Controlled Version Control | **PENDING** | Procedurally, no task in this breakdown stages, commits, pushes, or opens a pull request, and T084/T087 both explicitly forbid the agent from executing the intentionally-failing Git/CI workflow itself — that boundary is fully honored right now. This principle is nonetheless marked `PENDING`, not `PASS`, because its remit here includes confirming the human-controlled verification actually took place, and neither form of `SC-007`'s evidence substitutes for the other: automated category coverage (see above) does not demonstrate that a human maintainer actually ran the negative-validation scenario end-to-end, which is exactly what T084 alone records. It becomes `PASS` once T084's evidence is recorded, not before. |

**Two unresolved normative/mechanism decisions remain** (see "Pending / Blocked Items" above): the `futures-util` pin for WebSocket tests, and the PID-capture/liveness-probe mechanism for the abrupt-supervisor test. **One item awaits human execution, not a document change**: T084's CI-break evidence. Per the user's explicit instruction for this correction pass, `/speckit-implement` was **NOT** invoked. No production code, migration, configuration, or test file was created or modified while generating or correcting this task list — only `specs/001-slice-0-foundation/tasks.md` was written. This document does not claim implementation-readiness while these blockers remain; it stops here for human review.
...
- Task count changed from **87 to 88** in the third revision: one task was added (T084, the human-maintainer checkpoint recording `SC-007`'s actual execution evidence, distinct from T083's documentation-only runbook). Every task from the old `T084` onward was renumbered by +1; every cross-reference in this file was updated to match. **This (fourth) revision adds, removes, and renumbers no task — the count stays at 88**; it only corrects Constitution Principle VIII, `T046`'s scope, several inflated traceability identifiers (`SC-005`, `SC-008`, `SC-009`, `SC-007`, `SC-001` on the rows listed in the fourth-revision correction note above), the `SC-007` automated-coverage-vs-human-checkpoint distinction, and the third revision's own inaccurate claim that `T057`–`T059` were marked `PENDING`.
